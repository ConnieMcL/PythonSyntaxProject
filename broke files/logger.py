...
class StreamHandler(logging.StreamHandler):
    ...
    def emit(self, record):
        ...
        if record.levelno < logging.DEBUG or not _conf.use_color:
            print(record.level, end='', file=stream)
        else:
            # Import utils.console only if necessary and at the latest because
            # the import takes a significant time [#4649]
            from .utils.console import color_print
            if record.levelno < logging.INFO:
                color_print(record.level, 'magenta', end='', file=stream)
            elif record.levelno < logging.WARN:
                color_print(record.level, 'green', end='', file=stream)
            elif record.levelno < logging.ERROR:
                color_print(record.level, 'brown', end='', file=stream)
            else:
                color_print(record.level, 'red', end='', file=stream)
